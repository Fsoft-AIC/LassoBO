...
class Beale(SyntheticFunction):
    ...
    def __call__(self, x):
        ...
        assert x.ndim == 1
        assert np.all(x <= self.ub) and np.all(x >= self.lb)
        result = (1.5 - x[0] + x[0] * x[1])**2 + (2.25 - x[0] + x[0] * x[1]**2)**2 + (2.625 - x[0] + x[0] * x[1]**3)**2
        if self.negate:
            result = - result
        return result

class BealeExtend(SyntheticFunction):
    def __init__(self, dims=2, negate=False):
        assert dims % 2 == 0
        SyntheticFunction.__init__(
            self, 
...
class Rastrigin(SyntheticFunction):
    ...
    def __call__(self, x):
        ...
        if self.negate:
            result = - result
        return result
class SumSquared(SyntheticFunction):
    def __init__(self, dims=10, negate=False):
        SyntheticFunction.__init__(
            self, 
            dims, 
        ...
        self.coeff = np.sort(np.random.rand(dims)**2)[::-1]
        print(self.coeff)
