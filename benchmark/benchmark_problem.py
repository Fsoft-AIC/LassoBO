...
import re
import time
from benchmark.synthetic_function import Ackley, Branin, Hartmann, Levy, Rosenbrock, HartmannExtend, BealeExtend, Rastrigin, SumSquared
from benchmark.tracker import Tracker, save_results
from benchmark.rover_function import Rover
from benchmark.LassoBench import LassoBenchFunction
...
def get_problem(func_name, save_config, seed=2021):
    ...
    """
    if func_name in ['nasbench', 'rover', 'HalfCheetah', 'Walker2d', 'Hopper','Ant']:
        if func_name in ['HalfCheetah', 'Walker2d', 'Hopper','Ant']:
        ...
        if func_name == 'nasbench':
            ...
            return RLBenchmark(RLEnv('Ant-v2', seed), 888, list(range(888)), save_config)
        else:
            assert 0
        ...
        if func_name == 'nasbench1shot1':
            ...
            assert 0
    elif func_name in ['Breast_cancer', 'DNA', 'Diabetes', 'Leukemia',]:
        if func_name == 'DNA':
            return FunctionBenchmark(LassoBenchFunction(name = func_name), 180, list(range(180)), save_config)
    else:
        ...
        elif func_name.startswith('levy'):
            return FunctionBenchmark(Levy(valid_dims, True), dims, list(range(valid_dims)), save_config)
        elif func_name.startswith('ackley'):
            return FunctionBenchmark(Ackley(valid_dims, True), dims, list(range(valid_dims)), save_config)
        elif func_name.startswith('rastrigin'):
            return FunctionBenchmark(Rastrigin(valid_dims, True), dims, list(range(valid_dims)), save_config)
        elif func_name.startswith('rosenbrock'):
            return FunctionBenchmark(Rosenbrock(valid_dims, True), dims, list(range(valid_dims)), save_config)
        elif func_name.startswith('branin'):
            return FunctionBenchmark(Branin(valid_dims, True), dims, list(range(valid_dims)), save_config)
        elif func_name.startswith('beale'):
            return FunctionBenchmark(BealeExtend(valid_dims, True), dims, list(range(valid_dims)), save_config)
        elif func_name.startswith('sumsquared'):
            return FunctionBenchmark(SumSquared(valid_dims, True), dims, list(range(valid_dims)), save_config)
